...
dp83902a_RxEvent(void)
{
	...
	while (true) {
		...
		for (i = 0;  i < sizeof(rcv_hdr); i+=2) {
			DP_IN_DP(base, *(unsigned short*)(&rcv_hdr[i]));
		}
#if DEBUG & 5
...
static hw_info_t * get_prom(cyg_uint8 *base, uchar* mac) {
	...
	struct {
		...
		{DP_DCR_INIT,	EN0_DCFG},	/* Set byte-wide (0x48) access. */
	...
	for (i = 0; i < 32; i+=2) {
		...
		PRINTK(" %02x %02x", prom[i],prom[i+1]);
	}
...
void uboot_push_packet_len(int len) {
	...
	if (len>=2000) {
		printf("NE2000: packet too big (%i)\n",len);
		return;
	}
	if (nrx >= NB) {
